...
def execute(args: dict, context: dict) -> dict:
    ...
    if not get_or_create_sheet_tab:
        return {"saved": False, "time_str": "", "task": task, "due_date": due_date}

    try:
        from datetime import datetime
        time_str = datetime.now().strftime("%Y/%m/%d %H:%M")
        sheet = get_or_create_sheet_tab(TAB_NAME, HEADERS)
        sheet.append_row([time_str, task, due_date, status])
        return {"saved": True, "time_str": time_str, "task": task, "due_date": due_date, "status": status}
